...
exports.getVideos = async (req, res) => {
  try {
    ...
    const videos = await Video.find({ exam: { $in: examIds } })
      .populate("student", "name email studentId program level") // Populate student details (e.g., name, email)
      .populate("exam", "title program courseCode level") // Populate exam details
      .exec();
    let result = videos.length;
    if (videos.length === 0) {
      return res
        .status(404)
...
//     // Get the video file path of the first chunk (you can stream the first chunk as demo or all)
//     const videoPath = path.join(__dirname, "..", videoLinks[0]);
...
//     if (videoRange) {
//       // Parsing the range header for the start and end
//       const parts = videoRange.replace(/bytes=/, "").split("-");
//       const start = parseInt(parts[0], 10);
//       const end = parts[1] ? parseInt(parts[1], 10) : fileSize - 1;
//       const chunkSize = end - start + 1;
